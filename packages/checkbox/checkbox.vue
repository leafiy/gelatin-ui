...
export default {
  ...
  computed: {
    classes() {
      return {
        ...
        "ui-checkbox-indeterminate":
          this.$options.propsData.hasOwnProperty("indeterminate") &&
          this.indeterminate
      };
    },
    isChecked() {
    ...
    isGroup() {
      return (
        this.$options.propsData.hasOwnProperty("value") &&
  ...
  methods: {
    ...
    setValue(e) {
      if (this.disabled || this.$options.hasOwnProperty("indeterminate"))
        return;
      let value;
      if (!this.isGroup) {
