<template>
  <div class="ui-countdown" :class="[`ui-countdown-theme-${theme}`]">
    ...
    <div class="ui-countdown-date" v-if="date">
      <div class="ui-countdown-date-d" v-if="showDays">
        {{ days
        }}<slot name="day"><span class="ui-countdown-date-unit">天</span></slot>
      </div>
      <div class="ui-countdown-date-h" v-if="showHours">
        {{ hours
        }}<slot name="hour"
          ><span class="ui-countdown-date-unit">小时</span></slot
        >
      </div>
      <div class="ui-countdown-date-m" v-if="showMinutes">
        {{ minutes
        }}<slot name="minute"
          ><span class="ui-countdown-date-unit">分</span></slot
        >
      </div>
      <div class="ui-countdown-date-s" v-if="showSeconds">
        {{ seconds
        }}<slot name="second"
          ><span class="ui-countdown-date-unit">秒</span></slot
        >
      </div>
      <div class="ui-countdown-date-ms" v-if="showMs">
        {{ ms
        }}<slot name="ms"><span class="ui-countdown-date-unit"></span></slot>
      </div>
    </div>
    <slot></slot>
  </div>
